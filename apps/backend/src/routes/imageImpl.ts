...
export const listUserImages: AppRouteImplementation<typeof contract.image.listUserImages> = async ({
  params,
}) => {
  const { userId } = params;
...
export const pullAndUploadImage: AppRouteImplementation<
  typeof contract.image.pullAndUploadImage
> = async ({ body }) => {
